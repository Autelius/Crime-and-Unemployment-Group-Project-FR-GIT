...
author: |
  Romeo Schoonderbeek 2868407,  Paul Claessens 2859603,  
...
colnames(merged_panel_data_final) <- c("year", "province", "unemployment_rate",
                                       "total_crimes", "population", "crimes_per_capita", "area_km2")
```
...
```{r, echo=FALSE, message=FALSE, include=TRUE, warning=FALSE, fig.show='hold'}
# Crime per capita
p1 <- ggplot(nl_crime) +
  ...
  labs(title = "Crime per capita by Dutch province – 2023") +
...
p2 <- ggplot(nl_crime) +
  ...
  labs(title = "Population density by Dutch province – 2023") +
  theme_minimal() + theme_void()
